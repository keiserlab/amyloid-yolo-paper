...
if __name__ == "__main__":
    ...
    best_map, best_map_epoch = -1, -1
    best_cored_map, best_cored_map_epoch = -1, -1
    best_CAA_map, best_CAA_map_epoch = -1, -1
        ...
        if epoch % opt.evaluation_interval == 0:
            ...
            if metrics_output is not None:
                ...
                for i, c in enumerate(ap_class):
                    ...
                    if class_names[c] == "CAA": ##dan's edit
                        if AP[i] > best_CAA_map:
                            best_CAA_map = AP[i]
                            best_CAA_map_epoch = epoch
                    if class_names[c] == "Cored":
                        if AP[i] > best_cored_map:
                            best_cored_map = AP[i]
                            best_cored_map_epoch = epoch
                    if AP.mean() > best_map: 
                        best_map = AP.mean()
    ...
    print("Best mAP: {} @ epoch: {}".format(best_map, best_map_epoch))
    print("Best Cored mAP: {} @ epoch: {}".format(best_cored_map, best_cored_map_epoch))
    print("Best CAA mAP: {} @ epoch: {}".format(best_CAA_map, best_CAA_map_epoch))
